...
# Fast and cost-effective model choice for classification tasks
...
def classify_review(comment):
    # Configure Gemini with system instructions and enforce JSON output
...
def classify_with_retry(comment, retries=3, delay=2):
    for attempt in range(retries):
        try:
            return classify_review(comment)
...
def main():
    ...
    for review_id, comment in reviews:
        ...
        try:
            labels = classify_with_retry(comment)
